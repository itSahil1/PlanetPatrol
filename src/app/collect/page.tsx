...
export default function CollectPage() {
  ...
  const handleVerify = async () => {
    ...
    try {
      ...
      console.log('Verification Response:', sanitizedText)
      ...
      try {
        ...
        if (parsedResult.wasteTypeMatch && parsedResult.quantityMatch) {
          ...
          if (!parsedResult.wasteTypeMatch && !parsedResult.quantityMatch) {
            toast.error('Verification failed. Both waste type and quantity do not match.', {
              duration: 5000,
              position: 'top-center',
            })
          } else if (!parsedResult.wasteTypeMatch) {
            toast.error('Verification failed. The waste type does not match.', {
              duration: 5000,
              position: 'top-center',
            })
          } else if (!parsedResult.quantityMatch) {
            toast.error('Verification failed. The quantity does not match.', {
              duration: 5000,
              position: 'top-center',
            })
          }
        ...
        toast.error('Verification failed. Invalid response from verification service.', {
          duration: 5000,
          position: 'top-center',
        })
      }
